# Mentorship Program: Role Research
As part of the mentorship program, we explore the core requirements organizations prioritize when recruiting—skills, experience, certifications, and education—because these elements not only demonstrate a candidate’s ability to perform but also signal readiness for growth and alignment with industry standards. Understanding how these factors influence hiring decisions helps mentors guide participants toward building competitive profiles that stand out in today’s talent-driven market. <BR /> 

## Our goal for this role research:<BR /> 
Research security roles to determine required skills, certifications, experience, and educational requirements to provide visibility, in the current job landscape.<BR /> 

## Frequency of this research:<BR /> 
Annual research per role<BR /> 

## How is the research completed:<BR /> 
Researcher will obtain 30-50 JDs and review against expected criteria.<BR /> 
The greater the number of roles diseminated the greater the accuracy of the role research report.<BR /> 

## What roles we intend to research:
* Security Analyst - protects an organization's computer networks and system from cyber threats by monitoring for breaches, investigating incidents, implementing security measures and developing policies
* Security Engineer - designs, builds, and maintains secure IT systems and networks
* Security Manager - manages individuals, or groups of individual contributors but may also function as an analyst or engineer
* Security Architect - designs build and maintains an organizations overall cybersecurity structure
* Security Director - oversees all security operations within an organization, managing both physical and cybersecurity measures to protect people, assets, and data.


## What specifically are employers looking for:
* Skills - abilities possed by the individual required to complete their assigned job duties
* Certifications - credentials proving specific skills or knowledge
* Experience - knowledge or skill gained for doing, seeing, or feeling things over time
* Education - the lifelong process of acquiring knowledge, skills, values, and character development (usually form, non-formal, informal in nature)

## Role Research by Year
2025 [Security Analyst Role Research](https://github.com/bvoris/houseccon2025presentation/blob/main/research/Security%20Analyst%20Job%20Research%202025.xlsx)<BR /> 
2026 [Security Analyst Role Research](https://github.com/bvoris/mentorship-roleresearch/raw/refs/heads/main/2026/Security%20Analyst%20Job%20Research%202026.xlsx)<BR /> 
2026 [Security Engineer Role Research](https://github.com/bvoris/mentorship-roleresearch/blob/main/2026/Security%20Engineer%20Job%20Research%202026.xlsx)<BR /> 

## Connect with me at

<a href="https://twitter.com/HMInfoSecViking?ref_src=twsrc%5Etfw"><IMG SRC="https://github.com/bvoris/bvoris/blob/master/twitter.jpg" WIDTH=10% HEIGHT=10% ALIGN=LEFT></a>	
<a href="https://www.linkedin.com/in/brad-voris" target="_blank"><IMG SRC="https://github.com/bvoris/bvoris/blob/master/linkedin.png" WIDTH=10% HEIGHT=4% ALIGN=RIGHT></a>
<BR /> 
<A HREF="https://www.victimoftechnology.com">Victim Of Technology<A />
<BR /><BR />

